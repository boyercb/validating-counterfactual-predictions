...
\section{Time-fixed treatments} \label{sec:appendixa}
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            % &= \E\left(\frac{I(A = a)}{\Pr[A = a \mid X,D_{test} = 1]}\E\left[L\{Y, \widehat{\mu}(X^*)\} \mid X,D_{test} = 1\right] \mid D_{test} = 1\right)\\
            &= \E\left[\frac{I(A = a)}{\Pr[A = a \mid X,D_{test} = 1]}L\{Y, \widehat{\mu}(X^*)\} \mid D_{test} = 1\right]
        \end{align*}
        where the second line follows from the definition of conditional expectation, and the last reverses the law of iterated expectations.
    \end{proof}
    
    \subsection{Plug-in estimation for general loss functions}
    ...
    % \subsection{Inference for general loss functions}
    % Standard error estimates for $\widehat{\psi}_{CL}(a)$ and $\widehat{\psi}_{IPW}(a)$ are obtainable using resampling methods \cite{efron_introduction_1993}. Alternatively, estimating equations  \cite{robertson_regression-based_2021, efron_introduction_1993, huber1967behavior}.
    ...
    For plugin estimation for a binary treatment, the corresponding sample analogs are given by
    \begin{align}
...
% In the survival setting, the analyst typically builds a prediction model targeting the conditional cumulative incidence, $\Pr(T \leq t \mid X^*)$, conditional on covariates $X^*$ for some $t$ representing the desired prediction horizon. Note, as defined above, $\Pr(T \leq t \mid X^*) = \Pr(Y_{k+1} = 1 \mid X^*)$ when $k + 1 = t$. To determine the performance of fitted model $\widehat{\mu}(X^*)$ under a counterfactual regime $g$, we'd like to estimate the expected loss 
% $$\psi(a) = \E[L\{I(T^g\leq t), \widehat{\mu}(X^*)\}]$$
% for generalized loss function $L\{I(T^g\leq t), \widehat{\mu}(X^*)\}$. This is equivalent to $\E[L\{Y^g_{K+1}, \widehat{\mu}(X^*)\}]$ for suitably chosen $K$. Under right censoring, we can extend this to 
% $$\psi(a) = \E[L\{I(T^{g,\overline{c}=0}\leq t), \widehat{\mu}(X^*)\}]$$
% where $\overline{c}=0$ denotes the counterfactual outcome if no one was censored. This is again equivalent to $\E[L\{Y^{g,\overline{c}=0}_{K+1}, \widehat{\mu}(X^*)\}]$ for suitably chosen $K$.
...
% Under time-varying treatment initiation, the expected counterfactual loss for general loss function $L\{Y^{g, \overline{c}=0}_{K+1}, \widehat{\mu}(X^*)\}$ is identified by the functionals
%     \begin{align*}
%         \psi(a) = E_{X_0}\left\{E_{Y_1}\left(Y_1 + \ldots E_{Y_{K}}\left(Y_{K}(1 - Y_{K-1}) + E_{X_K}[L\{Y, \widehat{\mu}(X^*)\} \mid \overline{X}_K, \overline{A}_K=\overline{a}^g_K, D_{test} = 1] \right)\ldots \mid X_{0}, A_{0}=a^g_{0}, D_{test} = 1\right)\mid D_{test} = 1\right\}
%     \end{align*}
% and 
%     \begin{equation*}
%         \psi(a) = \E\left[\frac{I(\overline{A}_K = \overline{a}^g_K)}{\prod_{k=0}^K\Pr(A_k = a^g_k \mid \overline{X}_k, \overline{A}_{k-1} = \overline{a}^g_{k-1}, D_{test} = 1)}L\{Y, \widehat{\mu}(X^*)\} \mid D_{test} = 1\right]
%     \end{equation*}
...
%     For the first representation we have 
%     \begin{align*}
%         \psi(a) &= \E[L\{Y^{g}, \widehat{\mu}(X^*)\}] \\
%         & = \E[L\{Y^{g}, \widehat{\mu}(X^*)\}\mid D_{test} = 1] \\
%         & = \E(\E[L\{Y^{g}, \widehat{\mu}(X^*)\}\mid X_0, D_{test} = 1] \mid D_{test} = 1) \\
%         & = \E(\E[L\{Y^{g}, \widehat{\mu}(X^*)\}\mid X_0, A_0 = a^g_0, D_{test} = 1] \mid D_{test} = 1) 
%     \end{align*}
%     where the first line follows from the definition of $\psi(a)$, the second from random sampling of the test set, the third from the law of iterated expectations, and the fourth from the exchangeability condition. Arguing recursively from $k = 0$ to $K$, we can repeatedly invoke iterated expectations and exchanageability to insert $\overline{X}_k$ and $\overline{A}_k = \overline{a}^g_k$, such that
%     \begin{align*}
%         \psi(a) &= E\left\{\E\left(\ldots \E[L\{Y^g, \widehat{\mu}(X^*)\} \mid \overline{X}_K, \overline{A}_K=\overline{a}^g_K, D_{test} = 1]\ldots \mid X_{0}, A_{0}=a^g_{0}, D_{test} = 1\right)\mid D_{test} = 1\right\} \\
%         &= \E\left\{\E\left(\ldots \E[L\{Y, \widehat{\mu}(X^*)\} \mid \overline{X}_K, \overline{A}_K=\overline{a}^g_K, D_{test} = 1]\ldots \mid X_{0}, A_{0}=a^g_{0}, D_{test} = 1\right)\mid D_{test} = 1\right\}
%     \end{align*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        % &= \E\left[\frac{I(A = a)}{\Pr[A = a \mid X, \widehat{\mu}(x^*), D_{test} = 1]}\E\left\{ I(Y=1) \mid X, \widehat{\mu}(x^*), D_{test} = 1\right\}\mid \widehat{\mu}(x^*), D_{test} = 1\right]\\
